...
class AsianBartTokenizer(XLMRobertaTokenizer):
  ...
  def __init__(self,
    ...
    self.pretrained_vocab_files_map = {
        "vocab_file": {
            "hyunwoongko/asian-bart-ecjk":
                "https://huggingface.co/hyunwoongko/asian-bart-ecjk/resolve/main/sentencepiece.bpe.model",
            "hyunwoongko/asian-bart-en":
                "https://huggingface.co/hyunwoongko/asian-bart-en/resolve/main/sentencepiece.bpe.model",
            "hyunwoongko/asian-bart-ja":
                "https://huggingface.co/hyunwoongko/asian-bart-ja/resolve/main/sentencepiece.bpe.model",
            "hyunwoongko/asian-bart-ko":
                "https://huggingface.co/hyunwoongko/asian-bart-ko/resolve/main/sentencepiece.bpe.model",
            "hyunwoongko/asian-bart-zh":
                "https://huggingface.co/hyunwoongko/asian-bart-zh/resolve/main/sentencepiece.bpe.model",
        }
    }
  ...
  def prepare_seq2seq_batch(
    ...
    assert len(src_texts) == len(
        src_langs), "src_texts list and src_langs list must have same length"
    ...
    assert len(tgt_texts) == len(
        tgt_langs), "tgt_texts list and tgt_langs list must have same length"
...
class BartCgecTokenizer(AsianBartTokenizer):

  LANG = "zh_CN"

  def __init__(self, *args, tokenizer_file=None, lang=None, **kwargs):
    super().__init__(*args,
                     tokenizer_file=tokenizer_file,
                     src_lang=BartCgecTokenizer.LANG,
                     tgt_lang=BartCgecTokenizer.LANG,
                     **kwargs)
    self.decoder = False
  ...
  def __call__(
    ...
    langs = [BartCgecTokenizer.LANG] * len(texts)

    assert len(texts) == len(
        langs), "src_texts list and src_langs list must have same length"

    tokens = super().__call__(
        texts,
        padding=True,
        truncation=True,
        add_special_tokens=False,
    ...
    if self.decoder:
      tokens["input_ids"] = [[self.lang_code_to_id[BartCgecTokenizer.LANG]] +
                             input_ids + [self.eos_token_id] +
                             [self.pad_token_id] *
                             (max_length - len(input_ids) - 2)
                             for input_ids in tokens["input_ids"]]
    else:
      tokens["input_ids"] = [
          input_ids + [self.eos_token_id] +
          [self.lang_code_to_id[BartCgecTokenizer.LANG]] + [self.pad_token_id] *
          (max_length - len(input_ids) - 2) for input_ids in tokens["input_ids"]
      ]
    tokens["attention_mask"] = [
        attention_mask + [1] * 2 + [0] * (max_length - len(attention_mask) - 2)
